...
import { DepositBankAccountDto } from "../../../backendTypes/dto/DepositBankAccountDto";
...
import DateInput from "../../../components/inputs/DateInput/DateInput/DateInput";
...
function validateCreateDepositBankAccountForm(dto: DepositBankAccountDto) {
    const errors: ErrorType<DepositBankAccountDto> = {};
    if (isNil(dto.bankId)) {
    ...
    if (isNil(dto.dueDate)) {
        errors.dueDate = "Обязательное поле";
    }

    return errors;
}

function CreateDepositBankAccountForm() {
    const closeModal = useContext(ModalContext);

    const [createDebitBankAccount, isLoading] = useMutationQuery<DepositBankAccountDto>(
    ...
    const bankOptions: ISelectOption[] = banks
        ?.filter((bank) => bank.allowedBankAccountTypes.includes(BankAccountType.DEPOSIT))
    ...
    return (
        <Formik<DepositBankAccountDto>
            initialValues={{
                bankId: null,
                bankClientId: null,
                dueDate: new Date(),
            }}
            validate={validateCreateDepositBankAccountForm}
        ...
        >
            <FormLayout isDataLoading={isLoading} onCancel={closeModal}>
                ...
                <DateInput label="Дата окончания" fieldId="dueDate" />
            </FormLayout>
        </Formik>
    );
}

export default CreateDepositBankAccountForm;
